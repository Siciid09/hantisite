...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const expenseRef = await firestoreAdmin.collection("expenses").add(newExpense);

    // Optional: Log to Activity Feed if you use one
    /*
    await firestoreAdmin.collection("activity_feed").add({
      storeId,
      description: `Added Expense: ${currency} ${amount}`,
      userName,
      timestamp: new Date(),
    });
    */
    ...
    return NextResponse.json(
      { success: true, id: expenseRef.id },
      { status: 201 }
    );

  } catch (error: any) {
    console.error("[Expenses API POST] Error:", error.stack || error.message);
